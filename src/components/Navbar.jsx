 ...
 *
 * Características:
 * - Logo SVG vectorial diseñado con código (elegante, minimalista, botánico)
 * - Menú de navegación con scroll suave a secciones
 * - Indicador visual de carrito con badge animado
 * - Cambio de estilo al hacer scroll (efecto glassmorphism)
 * - Menú hamburguesa para mobile
 * - Accesibilidad: aria-labels, roles, manejo de teclado
 *
...
export default function Navbar({ cartCount = 0, onCartClick }) {
  ...
  // ── Detectar scroll para activar el glassmorphism ──────────────────────
  useEffect(() => {
  ...
  // ── Animar el carrito cuando se agrega un item ─────────────────────────
  useEffect(() => {
  ...
  // ── Cerrar el menú mobile al hacer scroll ──────────────────────────────
  useEffect(() => {
    if (isMobileOpen) {
          ...
          >
            ...
            <div className="relative w-9 h-9 flex-shrink-0">
              ...
              >
                ...
                {/* Hoja estilizada superior izquierda */}
                <path
                  d="M10 8 C10 8, 22 7, 24 18 C24 18, 14 17, 10 8Z"
                  fill="#C5A880"
                  opacity="0.85"
                />
                {/* Hoja estilizada inferior derecha */}
                <path
                  d="M26 28 C26 28, 14 29, 12 18 C12 18, 22 19, 26 28Z"
                  fill="#4A7060"
                  opacity="0.75"
                />
                {/* Tallo central */}
                <line
                  x1="18" y1="7" x2="18" y2="29"
                  stroke="#FAF8F5"
                  strokeWidth="1.2"
                  strokeLinecap="round"
                />
                {/* Puntos de energía botánica */}
            ...
            <div className="flex flex-col leading-none">
              <span className="font-display text-xl font-semibold tracking-[0.18em] text-sage uppercase">
                UMAY
              </span>
              <span className="font-sans text-[0.55rem] tracking-[0.22em] text-charcoal-muted uppercase">
                Botánica Andina
              </span>
            </div>
          </a>
              ...
              <li key={link.href}>
                <button
                  ...
                  className="font-sans text-sm font-medium text-charcoal-soft
                             hover:text-sage transition-colors duration-250
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="md:hidden p-2 rounded-lg text-charcoal-soft
                         hover:bg-sage-50 transition-colors duration-250
              ...
              <li className="pt-2">
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2
                             px-4 py-3 bg-sage text-cream
                             rounded-card font-sans text-base font-medium
                             hover:bg-sage-dark transition-colors duration-250"
                >
