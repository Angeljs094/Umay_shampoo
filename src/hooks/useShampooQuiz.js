/**
 * @fileoverview Hook principal: useShampooQuiz
...
function quizReducer(state, action) {
  switch (action.type) {
    ...
    case A.ADD_TO_CART: {
      ...
      // Si ya existe exactamente esa fórmula → incrementar qty
    ...
    case A.RESTART_QUIZ:
      return { ...INITIAL_STATE, cartItems: state.cartItems };
...
export function useShampooQuiz() {
  ...
  useEffect(() => {
    try { localStorage.setItem(CART_KEY, JSON.stringify(state.cartItems)); } catch { /* silencioso */ }
  }, [state.cartItems]);
