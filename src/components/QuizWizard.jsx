 ...
 *
 * Flujo: Welcome → preguntas → Skeleton Loader IA → Resultado con carrito.
 * La descripción de la fórmula es 100% dinámica: menciona cada ingrediente
 * top-3 por nombre, beneficio científico clave y la razón por la que fue
 * seleccionado según las respuestas del usuario.
 *
...
/**
 * Genera una descripción personalizada de la fórmula basada en los 3 ingredientes
 * top y las respuestas del usuario al quiz.
 *
 * @param {Array}  topIngredients - Top 3 ingredientes rankeados
 * @param {Object} answers        - Respuestas del quiz { STEP_HAIR_TYPE, STEP_GOAL, STEP_SCENT }
 * @returns {string} Párrafo descriptivo con fundamento botánico
 */
function buildFormulaDescription(topIngredients, answers) {
  if (!topIngredients || topIngredients.length < 1) return '';

  // Mapas de contexto para personalizar el texto según las respuestas
  ...
  // Razones específicas por ingrediente según el objetivo del usuario
  const reasonByGoal = {
    muna: {
      ...
      dandruff:   'actúa directamente sobre el hongo Malassezia con sus aceites esenciales (pulegona y mentol)',
    ...
    romero: {
      growth:     'inhibe la 5-alfa-reductasa (la enzima ligada a la caída capilar) según estudios en Skinmed Journal',
  ...
  // Construir párrafo por ingrediente
  const ingredientLines = topIngredients.slice(0, 3).map((item, idx) => {
    const id      = item.id;
    const ingr    = item.data;
    const reason  = reasonByGoal[id]?.[goal] || reasonByGoal[id]?.default || ingr.benefits[0];
    const rank    = ['como activo estrella', 'como complemento sinérgico', 'como potenciador final'][idx];
    return `la ${ingr.name} (${ingr.latin}) ${rank}, que ${reason}`;
  });
  ...
  return `Diseñada específicamente para un ${hairCtx} con el objetivo de ${goalCtx}, esta fórmula integra ${listStr}. Los tres activos trabajan en cadena: el primero prepara el cuero cabelludo, el segundo actúa en la fibra capilar y el tercero sella y potencia el resultado. Una combinación que tu cabello no obtendrá en ningún shampoo de estante.`;
}
...
function ResultScreen({ result, onAddToCart, onRestart, isAddedToCart }) {
  ...
  // Descripción dinámica con ingredientes nombrados
  ...
  return (
    <div className="animate-fade-in-up">
      ...
      <div className="flex justify-center mb-8"
           aria-label={`Barra de shampoo personalizada color ${colorHex}`}>
      ...
      <div className="p-4 bg-cream rounded-botanical border border-sage-100 mb-4">
        ...
        <div className="flex items-center gap-1.5 mb-2">
          ...
          <span className="font-sans text-[10px] font-semibold text-gold-dark
            ...
            <div key={item.id} className="flex items-start gap-2.5">
              ...
              <div>
                <p className="font-sans text-[11px] font-semibold text-charcoal leading-tight">
                  {item.data.name}
                  <span className="font-normal text-charcoal-muted italic ml-1">
                    · {item.data.latin}
                  </span>
                </p>
      ...
      <div className="space-y-3">
        <button
          ...
          aria-label={isAddedToCart ? 'Producto ya en el carrito' : 'Agregar al carrito de compras'}
